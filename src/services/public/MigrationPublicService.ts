...
export class MigrationPublicService implements TMigrationPrivateService {
    ...
    readonly migrationPrivateService = inject<MigrationPrivateService>(TYPES.migrationPrivateService);
    ...
    public createProduct = async (title: string, description: string) => {
        ...
        return await ContextService.runInContext(async () => {
            return await this.migrationPrivateService.createProduct(title, description);
        }, {
